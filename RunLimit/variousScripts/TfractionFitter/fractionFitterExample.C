{
	...
	TH1F *mc0;                               // right handed helicity
	TH1F *mc1;                               // left handed helicity
	TH1F *mc2;                               // longitudinal helicity
	...
	// input template root files
	TFile *left = new TFile("/home/gouranga/Documents/NISER/work/top-related/helicity/technicalities/helicity_template/left/templates_ttbar_left.root");
	TFile *right = new TFile("/home/gouranga/Documents/NISER/work/top-related/helicity/technicalities/helicity_template/right/templates_ttbar_right.root");
	TFile *longitudinal = new TFile("/home/gouranga/Documents/NISER/work/top-related/helicity/technicalities/helicity_template/longitudinal/templates_ttbar_longitudinal.root");
	// contribution 0
	...
	// right handed histogram

	TH1F *h0 = (TH1F*)(right->Get("ee_mlb/central"))->Clone("h0");
	...
	TH1F *h1 = (TH1F*)(left->Get("ee_mlb/central"))->Clone("h1");
	...
	TH1F *h2 = (TH1F*)(longitudinal->Get("ee_mlb/central"))->Clone("h2");
	h2->SetLineColor(kBlue);
	h2->SetLineWidth(2);
	
	// generate data
	//data = new TH1F("data", "Data angle distribution", nBins, 0, TMath::Pi());
	data = new TH1F("data", "Data angle distribution", 20, 0.0, 200.0);
	data->SetXTitle(h0->GetXaxis()->GetTitle()); // directly get the title from input histogram
	...
	for( Int_t i=0; i<Ndata; i++) {
		...
		if( p<trueP0 ) { 
		  //x = f0->GetRandom();
		  x = h0->GetRandom(); // right handed template hist
		  htruemc0->Fill(x);
		}
		else if( p<trueP0+trueP1 ) { 
		  //x = f1->GetRandom();
		  x = h1->GetRandom(); // left handed template hist
		  htruemc1->Fill(x);
		}
		else { 
		  //x = f2->GetRandom();
		  x = h2->GetRandom(); // longitudinal template hist
		  htruemc2->Fill(x);
	...
	// debug 0
	// data entry
	std::cout << "data entry: " << data->GetEntries() << endl;
	
	// generate MC samples
	//mc0 = new TH1F("mc0", "MC sample 0 angle distribution", nBins, 0, TMath::Pi());
	mc0 = new TH1F("mc0", "MC sample 0 angle distribution", 20, 0.0, 200.0);
	mc0->SetXTitle(h0->GetXaxis()->GetTitle());
	mc0->SetLineColor(2);
	...
	for( Int_t i=0; i<N0; i++) {
	  ...
	  mc0->Fill( h0->GetRandom() );
	}

	//mc1 = new TH1F("mc1", "MC sample 1 angle distribution", nBins, 0, TMath::Pi());
	mc1 = new TH1F("mc1", "MC sample 1 angle distribution", 20, 0.0, 200.0);
	mc1->SetXTitle(h0->GetXaxis()->GetTitle());
	mc1->SetLineColor(3);
	...
	for( Int_t i=0; i<N1; i++) {
	  ...
	  mc1->Fill( h1->GetRandom() );
	}

	//mc2 = new TH1F("mc2", "MC sample 2 angle distribution", nBins, 0, TMath::Pi());
	mc2 = new TH1F("mc2", "MC sample 2 angle distribution", 20, 0.0, 200.0);
	mc2->SetXTitle(h0->GetXaxis()->GetTitle());
	mc2->SetLineColor(4);
	...
	for( Int_t i=0; i<N2; i++) {
	  ...
	  mc2->Fill( h2->GetRandom() );
	}


	mc0->Scale(1.0/mc0->Integral());
	mc1->Scale(1.0/mc1->Integral());
	mc2->Scale(1.0/mc2->Integral());

	std::cout << "mc0->Integral():  " << mc0->Integral() << std::endl;
	std::cout << "mc1->Integral():  " << mc1->Integral() << std::endl;
	std::cout << "mc2->Integral():  " << mc2->Integral() << std::endl;
	// Fitter warning
	// FractionFitter
	...
	h0->DrawClone(); // DrawClone does not work on MAC
	//h0->GetHistogram()->SetTitle("Original MC distributions"); // commented out
	h1->DrawClone("same");
	h2->DrawClone("same");
	...
	//data->Draw("EP");
	data->DrawClone("EP"); // DrawClone does not work here on MAC
	htruemc0->Draw("same");
	htruemc1->Draw("same");
	htruemc2->Draw("same");
